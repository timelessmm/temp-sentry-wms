<div align="center">
  ...
  ![Version](https://img.shields.io/badge/version-1.10.1-8e2716)
  ![Tests](https://img.shields.io/badge/tests-2367%20passing-34a853)
...
**v1.10.1 - Patch release on top of the v1.10.0 POS surface. Two operator-experience changes. The admin token validator now accepts the `dockd.dispatch` and `pos.dispatch` slugs in `CreateToken` / `UpdateToken` requests; prior to this fix `POST /api/admin/tokens` returned 400 `unknown_endpoint_slugs` on those scopes even though the auth middleware honored them at request time, blocking operators from issuing dockd or POS tokens through the admin API. The admin Imports page gains a fifth tab for bulk inventory-adjustment CSV upload (#329): `sku`, `warehouse`, `bin`, `qty` (signed integer), `memo` columns; each accepted row writes an `inventory_adjustments` row with `reason_code='CORRECTION'`, `status='APPROVED'` so the on-hand change applies inline, with `audit_log` (`ACTION_ADJUST`) and `adjustment.applied/1` outbox event fired per accepted row. No migrations.**
...
Built by [Hightower Systems L.L.C.](https://github.com/hightower-systems) · v1.10.1
